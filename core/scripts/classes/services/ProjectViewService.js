			...
			_base.initializeClient(function() {
				...
				function getProject() {
					...
					_base.getFromCache(cacheKey, function(value) {
						...
						options.projectName = request.params.name;

						repo.getProjectByName(_base.getClient(), options,
							function(x) {
							...
							function() {
								...
								_base.addError(new Error('Projet not found',
									404));
								deferred.reject();
							});
					});
				...
				function computeData(website, pages, menuEvents,
					...
					for (var i = 0; i < viewData.projectBreadcrumbPages.length; i++) {
						data.breadcrumbPages.push(viewData.projectBreadcrumbPages[
							i]);
					}
